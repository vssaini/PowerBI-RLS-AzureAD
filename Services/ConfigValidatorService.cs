namespace PowerBiRls.Services
{
    ...
    public class ConfigValidatorService
    {
        ...
        public static string ValidateConfig(IOptions<AzureAd> azureAd, IOptions<PowerBI> powerBI)
        {
            ...
            else if (string.IsNullOrWhiteSpace(azureAd.Value.Instance))
            {
                message = "Instance is not set in appsettings.json file";
            }
            else if (string.IsNullOrWhiteSpace(azureAd.Value.CallbackPath))
            {
                message = "CallbackPath is not set in appsettings.json file";
            }
